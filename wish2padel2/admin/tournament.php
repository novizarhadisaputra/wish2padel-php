...
if (isset($_POST['add_zone'])) {
    ...
    try {
        ...
        // 🟡 Tambahan: registration_until
        $registration_until_input = $_POST['registration_until'] ?? null;
        $registration_until = $registration_until_input 
            ? date('Y-m-d', strtotime($registration_until_input))
            : null;
    
        jslog('STEP 1: INPUT', compact('name','start_date','end_date','id_league','registration_until','status'));
    
        // =======================
        // STEP 2: INSERT TOURNAMENT BARU
        // =======================
        $stmt = $conn->prepare("
            INSERT INTO tournaments (name, start_date, end_date, registration_until, id_league, status)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("ssssis", $name, $start_date, $end_date, $registration_until, $id_league, $status);
        $stmt->execute();
        $new_tournament_id = (int)$stmt->insert_id;
        $stmt->close();
    
        jslog('STEP 2: INSERTED new tournament', [
            'new_tournament_id'=>$new_tournament_id,
            'registration_until'=>$registration_until
        ]);

        // =======================
        // STEP 3: CARI TURNAMEN LAMA (nama sama, league sama, id < baru, ambil paling baru sebelumnya)
        // =======================
        $oldTournament = $conn->prepare("
            ...
            LIMIT 1
        ");
        $oldTournament->bind_param("sii", $name, $id_league, $new_tournament_id);
        ...
        $oldTournament->close();
        jslog('STEP 3: FOUND old tournament', [
...
if (isset($_POST['edit_zone'])) {
    ...
    $registration_until_input = $_POST['registration_until'];
    $start_date_input         = $_POST['start_date'];
    $end_date_input           = $_POST['end_date'];

    $registration_until = date('Y-m-d', strtotime($registration_until_input));
    ...
    $stmt = $conn->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->bind_param("ssssii", $name, $registration_until, $start_date, $end_date, $id_league, $id);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['delete_zone'])) {
    ...
    // Redirect balik ke halaman sendiri (tanpa echo JS)
...
<?php 
// Reset pointer agar bisa dibaca ulang untuk generate modals di luar section
$leagues->data_seek(0); 
    ...
    <div class="modal-content shadow-lg rounded-4 border-0">
      <div class="modal-header bg-warning text-dark rounded-top-4">
        <h5 class="modal-title">
          <i class="bi bi-pencil-square me-2"></i>Update League
        </h5>
...
<section class="p-4">
  <div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn-gold shadow-sm rounded-pill" data-bs-toggle="modal" data-bs-target="#addZoneModal">
          ...
          <table class="table table-hover align-middle">
            ...
            <tbody>
              <?php if (empty($tournaments)): ?>
                <tr><td colspan="7" class="text-center text-muted">No zones found.</td></tr>
              <?php else: ?>
                <?php foreach($tournaments as $row): ?>
                  <tr>
                    <td class="fw-semibold"><?= htmlspecialchars($row['name']) ?></td>
                    <td class="text-muted"><?= htmlspecialchars($row['league_name'] ?? '-') ?></td>
                    <td><span class="badge bg-warning text-dark px-3 py-2 rounded-pill"><?= $row['registration_until'] ?: '-' ?></span></td>
                    ...
                    <td class="text-center">
                      ...
                      <button class="btn btn-sm btn-outline-warning me-1 rounded-circle" 
                              data-bs-toggle="modal" 
                              data-bs-target="#editZoneModal<?= $row['id'] ?>">
                        <i class="bi bi-pencil"></i>
                      </button>
                      ...
                      <button class="btn btn-sm btn-outline-danger rounded-circle" 
                              data-bs-toggle="modal" 
                              data-bs-target="#deleteZoneModal<?= $row['id'] ?>">
                        <i class="bi bi-trash"></i>
                      </button>
                    </td>
                  </tr>
...
<!-- ======================= Edit Zone Modal ======================= -->
<div class="modal fade" id="editZoneModal<?= $id ?>" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow-lg border-0 rounded-4">
      ...
      <form method="POST">
        <div class="modal-body">
            ...
            <select name="id_league" class="form-select" required>
              ...
              <?php foreach($leagues as $league): ?>
                <option value="<?= $league['id'] ?>" 
                        <?= $row['id_league']==$league['id'] ? 'selected' : '' ?>>
          ...
          <!-- Registration Until BEFORE Start Date -->
          <div class="form-floating mb-3">
            <input type="date" name="registration_until" class="form-control" 
                   value="<?= $row['registration_until'] ?>" required>
            <label>Registration Until</label>
          </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" name="edit_zone" class="btn-gold">Save Changes</button>
        </div>
      </form>
    </div>
  </div>
...
<!-- ======================= Delete Zone Modal ======================= -->
<div class="modal fade" id="deleteZoneModal<?= $id ?>" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content shadow-lg border-0 rounded-4">
      <div class="modal-header bg-danger text-white rounded-top-4">
        <h5 class="modal-title"><i class="bi bi-trash3 me-2"></i>Delete Zone</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form method="POST">
        <div class="modal-body">
          Are you sure you want to <strong class="text-danger">delete</strong> 
          zone "<strong><?= htmlspecialchars($row['name']) ?></strong>"?
          <input type="hidden" name="id" value="<?= $id ?>">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" name="delete_zone" class="btn btn-danger">
  ...
  <div class="modal-dialog modal-dialog-centered" style="margin-top:100px">
    <div class="modal-content shadow-lg border-0 rounded-4">
      <div class="modal-header text-dark rounded-top-4">
        <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Add Zone</h5>
      ...
      <form method="POST">
        <div class="modal-body">
          ...
          <!-- Registration Until BEFORE Start Date -->
          <div class="form-floating mb-3">
            <input type="date" name="registration_until" class="form-control" required>
            <label>Registration Until</label>
          </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" name="add_zone" class="btn-gold">
            <i class="bi bi-check-circle me-1"></i> Add Zone
          </button>
        </div>
      </form>
    </div>
              ...
              ?>
                <tr>
                  ...
                  <td class="text-center">
                    <button class="btn btn-sm btn-outline-warning me-1 rounded-circle" 
                            data-bs-toggle="modal" 
                            data-bs-target="#editDivisionModal<?= $row['id'] ?>" 
                            title="Edit">
                      <i class="bi bi-pencil"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger rounded-circle" 
                            data-bs-toggle="modal" 
                            data-bs-target="#deleteDivisionModal<?= $row['id'] ?>" 
                            title="Delete">
                      <i class="bi bi-trash"></i>
                    </button>
                  </td>
...
<div class="modal fade" id="editDivisionModal<?= $id ?>" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered" style="margin-top:70px">
    <div class="modal-content shadow-lg rounded-4 border-0">
      <div class="modal-header bg-warning text-dark rounded-top-4">
        <h5 class="modal-title">
          <i class="bi bi-pencil-square me-2"></i>Update Division
        </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-floating mb-3">
            <input type="number" name="id_new" class="form-control"
                   value="<?= $id ?>" placeholder="Division ID" required>
