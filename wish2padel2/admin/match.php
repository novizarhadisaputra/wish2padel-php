...
<section class="py-5">
  <div class="container">
    ...
    <div class="card shadow-sm border-0 rounded-3 mb-4">
      ...
      <!-- Delete All Matches By Year (optional, dikomen dulu) -->
      <!--
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle">
              ...
              <tbody>
                <?php 
                $no=1; 
                $modalData = []; // <-- simpan data modal untuk dirender di luar section
                ...
                <?php 
                // Simpan data modal untuk dirender nanti (di luar section)
                $modalData[] = $row;
                endforeach; 
                ?>
              </tbody>
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" 
                   value="<?= htmlspecialchars($row['league_name'] ?? '-') ?> / <?= htmlspecialchars($row['tournament_name'] ?? '-') ?>" disabled>
          </div>
          ...
          <div class="mb-3">
            ...
            <select name="team1_id" class="form-select" required>
              ...
              while($t = $teams->fetch_assoc()): ?>
                <option value="<?= $t['id'] ?>" <?= $t['id']==$row['team1_id']?'selected':'' ?>>
          ...
          <div class="mb-3">
            ...
            <select name="team2_id" class="form-select" required>
              ...
              while($t2 = $teams2->fetch_assoc()): ?>
                <option value="<?= $t2['id'] ?>" <?= $t2['id']==$row['team2_id']?'selected':'' ?>>
                  <?= htmlspecialchars($t2['team_name']) ?>
                </option>
              <?php endwhile; ?>
            </select>
          </div>
          ...
          <div class="mb-3">
            ...
            <input type="number" disabled name="journey" class="form-control" value="<?= $row['journey'] ?? 1 ?>">
          </div>

          <div class="mb-3">
            <label class="form-label">Scheduled Date & Time</label>
            <input type="datetime-local" name="scheduled_date" class="form-control" 
                   value="<?= date('Y-m-d\TH:i', strtotime($row['scheduled_date'])) ?>" required>
          </div>
