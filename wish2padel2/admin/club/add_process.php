...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $uploadDir = realpath(__DIR__ . '/../../uploads/club');
    if (!$uploadDir) {
        mkdir(__DIR__ . '/../../uploads/club', 0777, true);
        $uploadDir = realpath(__DIR__ . '/../../uploads/club');
    }
    ...
    if (!empty($_FILES['logo']['name']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
        ...
        $logo_path = $uploadDir . DIRECTORY_SEPARATOR . $logo_name;
        if (move_uploaded_file($_FILES['logo']['tmp_name'], $logo_path)) {
            $logo_url = $logo_name;
        } else {
            echo "<div style='color:red;'>❌ Gagal memindahkan file logo ke $logo_path</div>";
        }
    }

    // 🔍 Debug kecil
    // echo "<pre>Logo URL: $logo_url</pre>"; exit;
    ...
    // Insert ke PISTAS
    // =====================
    if (!empty($_POST['pista_type']) && !empty($_POST['pista_quantity'])) {
        foreach ($_POST['pista_type'] as $index => $type) {
            $quantity = $_POST['pista_quantity'][$index] ?? '';
            if (!empty($type) && !empty($quantity)) {
    ...
    if (!empty($_FILES['photos']['name'][0])) {
        foreach ($_FILES['photos']['name'] as $index => $photo_name) {
            if ($_FILES['photos']['error'][$index] === 0) {
                ...
                $upload_path = $uploadDir . DIRECTORY_SEPARATOR . $new_name;
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="../../assets/css/stylee.css?v=13">
  </head>
  <body style="background-color: #303030">
          ...
          <form action="" method="post" enctype="multipart/form-data" class="row g-3">
            ...
            <div class="col-12 mt-4 border rounded-3 p-3">
              ...
              <div id="pistas-wrapper">
                <div class="row g-2 mb-2 pista-item">
                  <div class="col-md-6">
                    <select name="pista_type[]" class="form-select">
            ...
            <div class="col-12 mt-4 text-end">
              <button type="submit" class="btn-gold px-4">
                <i class="bi bi-check-circle me-2"></i> Save Club
              </button>
            </div>
          </form>
        </div>
    ...
    <script>
      document.getElementById("add-pista").addEventListener("click", function () {
