...
<section class="mt-4 container">
    <div class="row justify-content-center">
        <div class="col-lg-10 mt-4 mb-5">
            ...
            <?php 
            $highlightSet = false; // highlight next match
            while ($m = $matches_res->fetch_assoc()):
                ...
                $scoreText = ($m['status'] === 'completed') ? "FT" : 
                             (($m['status'] === 'scheduled' || $m['status'] === 'pending') ? "0 - 0" : "VS");
                ...
                $lineup = $conn->query("
                    SELECT letter FROM lineup_letters 
                    WHERE match_id = {$m['id']} AND team_id = $team_id
                    ORDER BY uploaded_at DESC LIMIT 1
                ")->fetch_assoc()['letter'] ?? null;

                $result = $conn->query("
                    SELECT letter FROM match_results 
                    WHERE match_id = {$m['id']} AND team_id = $team_id
