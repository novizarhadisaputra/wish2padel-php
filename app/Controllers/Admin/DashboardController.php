...
class DashboardController
{
    public function index()
    {
        ...
        // Dashboard Stats
        $stats = [
            'leagues' => $conn->query("SELECT COUNT(*) AS total FROM league")->fetch_assoc()['total'],
            'zones' => $conn->query("SELECT COUNT(DISTINCT name) AS total FROM tournaments")->fetch_assoc()['total'],
            'news' => $conn->query("SELECT COUNT(*) AS total FROM blog_news")->fetch_assoc()['total'],
            'matches' => $conn->query("SELECT COUNT(*) AS total FROM matches")->fetch_assoc()['total'],
            'sponsors' => $conn->query("SELECT COUNT(*) AS total FROM sponsors")->fetch_assoc()['total'],
            'teams' => $conn->query("SELECT COUNT(DISTINCT team_name) AS total FROM team_info")->fetch_assoc()['total'],
            'clubs' => $conn->query("SELECT COUNT(*) AS total FROM centers")->fetch_assoc()['total'],
            'players' => $conn->query("SELECT COUNT(DISTINCT player_name) AS total FROM team_members_info")->fetch_assoc()['total'], // approximated
            'completed_matches' => $conn->query("SELECT COUNT(DISTINCT match_id) AS total FROM match_results WHERE status='accept'")->fetch_assoc()['total']
        ];
